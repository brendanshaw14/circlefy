...
const Circle = ({ color = "#000000", x = 0, y = 0, size = 0.1, image, text, scale = 1 }) => {
    ...
    const actualSize = Math.min(visualViewport.height, visualViewport.width) * (size / 100);

    return (
        <svg style={{ position: 'relative', left: actualX, top: actualY }} width={actualSize} height={actualSize}>
            <defs>
