...
const PopCircle = ({ color = "#000000", x = 0, y = 0, size = 0.1, image, text, scale = 1.1, delay = 0, label, song, artist, clickHandler}) => {
    ...
    return (
        ...
        <div style={{ textAlign: 'center', justifyContent: 'center', width: '0px', height: '0px' }}>
            <svg className="pop-circle" style={{ ...animationStyle, position: 'absolute', left: actualX-actualSize/2, top: actualY-actualSize/2}} width={actualSize} height={actualSize} onClick={clickHandler ? () => clickHandler(song) : undefined}>
                <defs>
